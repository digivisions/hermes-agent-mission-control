...
interface ProjectRow {
  ...
  hermesProfile: string | null; tags: string[]; overview: string | null;
...
export default function ProjectWorkspace({ params }: { params: Promise<{ project: string }> }) {
  const { project: slug } = use(params);
  const [data, setData] = useState<{ project: ProjectRow; approvals: Req[]; runs: Run[]; tasks: Task[] } | null>(null);
  ...
  if (notFound) return (
    <div className="pt-10">
      <EmptyState icon={<FolderKanban className="w-6 h-6" />} title={`No project '${slug}' in the registry`}
        hint="Add it to prisma/seed-projects.ts and re-run npm run db:seed:projects on the VPS." />
    </div>
  );

  const p = data?.project;
  const live = !!(p && p.hermesProfile);
  ...
  return (
    <div className="relative z-10 w-full mx-auto pb-10 flex flex-col" style={{ minHeight: "calc(100vh - 2rem)" }}>
      <div className="hq-rise pt-4 pb-6">
        <a href="/projects" className="inline-flex items-center gap-1.5 text-[12.5px] text-[var(--text-3)] hover:text-[var(--text-2)] transition-colors">
        ...
        <div className="mt-2.5 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            {p?.description && <p className="mt-2 text-[12.5px] text-[var(--text-2)] max-w-xl">{p.description}</p>}
          </div>
          <div className="flex items-center gap-2">
            {p && <Pill tone="neutral">{label("status", p.status)}</Pill>}
            {p && <Pill tone="accent">{label("priority", p.priority)}</Pill>}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start flex-1 min-h-0">
        <div className="xl:col-span-2 h-full min-h-0" style={{ minHeight: "62vh" }}>
          <Panel className="h-full min-h-0 flex flex-col p-0 overflow-hidden">
            <ClientChatThread
              client={slug}
              basePath={`/api/projects/${slug}`}
              disabled={false}
              disabledReason={`No Hermes profile for ${p?.name ?? slug} yet — run scripts/provision-profile.sh ${slug} on the VPS.`}
            />
          </Panel>
        </div>
        ...
        <div className="xl:col-span-1 flex flex-col gap-5">
          ...
          <div>
            ...
            {data && data.approvals.length === 0 ? (
              <Panel className="p-2"><EmptyState title="Nothing needs you for this project." /></Panel>
            ) : (
          ...
          {/* 2 — Recent agent runs: doubles as the per-project audit log */}
          <div>
          ...
          {/* File Center: real files from the project's repo on the Mac (SSH) */}
          <FileCenter basePath={`/api/projects/${slug}/files`} />
      ...
      {editing && p && (
        <ProjectEditor mode="edit" initial={p as unknown as Partial<ProjectCardLike>} onClose={() => setEditing(false)} onSaved={load} />
      )}
    </div>
  );
}
